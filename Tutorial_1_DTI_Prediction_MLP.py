...
def main(
    ...
    for experiment_id in range(num_samples):
        ...
        while not unseen_config_found:
            temp_config.update(
                {
                    ...
                    if param_name
                    not in [
                        "mlp_drug_nodes_per_layer",
                        "mlp_target_nodes_per_layer",
                        "cls_hidden_dims",
                    ]
                }
            )
            ...
            temp_config["cls_hidden_dims"] = get_sizes_per_layer(
                cls_num_layers, ranges_dict["cls_hidden_dims"], bottleneck=True
            )
            ...
            if completed_param_combinations_df[
                ...
                & (
                    completed_param_combinations_df["hidden_dim_drug"]
                    == temp_config["hidden_dim_drug"]
                )
                & (
                    completed_param_combinations_df["hidden_dim_protein"]
                    == temp_config["hidden_dim_protein"]
                )
                & (
                    completed_param_combinations_df["cls_hidden_dims"].apply(
                        (temp_config["cls_hidden_dims"]).__eq__
                    )
                )
                & (
