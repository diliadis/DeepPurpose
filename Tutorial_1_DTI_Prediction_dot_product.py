...
def main(num_samples, val_setting, cuda_id, num_workers, dataset_name, performance_threshold=1.0):
    ...
    for run in tqdm(runs):
        if run.state == "finished":
            if ((run.config['general_architecture_version'] == general_architecture_version) and (run.config['dataset_name'] == dataset_name) and (run.config['val_setting'] == val_setting)):
